...
const crumbs = computed(() => {
  const out: { label: string, query: Record<string, string | undefined> }[] = [
    { label: 'All recipes', query: {} }
  ]
  if (activeCategory.value !== 'All') {
    out.push({ label: activeCategory.value, query: { category: activeCategory.value } })
  }
  if (activeTag.value) {
    out.push({
  ...
  if (search.value) {
    out.push({ label: `"${search.value}"`, query: { ...route.query as Record<string, string> } })
  }
  return out
})
